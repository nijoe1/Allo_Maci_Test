...
class WitnessTester {
    ...
    async expectConstraintPass(witness) {
     ...
     */
    async expectConstraintFail(witness) {
        await this.expectConstraintPass(witness).then(() => chai_1.assert.fail('Expected constraints to not match.'), err => {
            // console.log(err.message);
            (0, chai_1.expect)(err.message).to.eq("Constraint doesn't match");
        });
    }
     ...
     */
    async expectConstraintCount(expected, exact) {
        const count = await this.getConstraintCount();
        (0, chai_1.expect)(count, 'Circuit is under-constrained').to.be.greaterThanOrEqual(expected);
        if (exact) {
            (0, chai_1.expect)(count, 'Circuit is over-constrained').to.eq(expected);
        }
    }
     ...
     */
    async expectFail(input) {
        return await this.calculateWitness(input).then(() => chai_1.assert.fail('Expected witness calculation to fail.'), err => {
            ...
            (0, chai_1.expect)(isExpectedError).to.be.true;
            return errorMessage;
        });
    }
     ...
     */
    async editWitness(witness, symbolValues) {
    ...
    async readWitness(witness, symbols) {
        await this.loadSymbols();
        const ans = {};
     ...
     */
    async readWitnessSignals(witness, signals) {
        await this.loadSymbols();
...
exports.WitnessTester = WitnessTester;
//# sourceMappingURL=witnessTester.js.map