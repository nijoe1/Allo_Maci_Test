 ...
 */
const genProofs = async ({ outputDir, tallyFile, tallyZkey, processZkey, pollId, subsidyFile, subsidyZkey, rapidsnark, processWitgen, processDatFile, tallyWitgen, tallyDatFile, subsidyWitgen, subsidyDatFile, coordinatorPrivKey, maciAddress, transactionHash, processWasm, tallyWasm, subsidyWasm, useWasm, stateFile, startBlock, blocksPerBatch, endBlock, signer, tallyAddress, useQuadraticVoting = true, quiet = true, }) => {
    ...
    let subsidyVk;
    if (subsidyFile) {
        if (fs_1.default.existsSync(subsidyFile)) {
            (0, utils_1.logError)(`${subsidyFile} exists. Please specify a different filepath.`);
        }
        if (!subsidyZkey) {
            (0, utils_1.logError)("Please specify the subsidy zkey file location");
        }
        if (!subsidyWitgen) {
            (0, utils_1.logError)("Please specify the subsidy witnessgen file location");
        }
        ...
        if (!useWasm) {
            ...
            const subsidyWitgenResult = (0, utils_1.doesPathExist)([subsidyWitgen, subsidyDatFile]);
            if (!subsidyWitgenResult[0]) {
                (0, utils_1.logError)(`Could not find ${subsidyWitgenResult[1]}.`);
            }
        }
        else {
            // we expect to have the wasm file
            if (!subsidyWasm) {
                (0, utils_1.logError)("Please specify the subsidy wasm file location");
            }
            const subsidyWasmResult = (0, utils_1.doesPathExist)([subsidyWasm]);
            if (!subsidyWasmResult[0]) {
                (0, utils_1.logError)(`Could not find ${subsidyWasmResult[1]}.`);
            }
        }
        // either way we check the subsidy zkey
        const subsidyZkeyResult = (0, utils_1.doesPathExist)([subsidyZkey]);
        if (!subsidyZkeyResult[0]) {
            (0, utils_1.logError)(`Could not find ${subsidyZkeyResult[1]}.`);
        }
        subsidyVk = await (0, maci_circuits_1.extractVk)(subsidyZkey);
    }
    ...
    if (stateFile) {
        ...
        maciState = await (0, maci_contracts_1.genMaciStateFromContract)(signer.provider, await maciContract.getAddress(), coordinatorKeypair, pollId, fromBlock, blocksPerBatch, endBlock);
    }
    ...
    if (subsidyFile) {
        ...
        (0, utils_1.logYellow)(quiet, (0, utils_1.info)(`Generating proofs of subsidy calculation...`));
        ...
        (0, utils_1.logYellow)(quiet, (0, utils_1.info)(`subsidyBatchSize=${subsidyBatchSize}, numLeaves=${numLeaves}, totalSubsidyBatch=${totalSubsidyBatches}`));
        ...
        while (poll.hasUnfinishedSubsidyCalculation()) {
            ...
            try {
                ...
                // eslint-disable-next-line no-await-in-loop
                const isValid = await (0, maci_circuits_1.verifyProof)(r.publicSignals, r.proof, subsidyVk);
                if (!isValid) {
                    (0, utils_1.logError)("Error: generated an invalid subsidy calc proof");
                }
                const thisProof = {
                ...
                fs_1.default.writeFileSync(path_1.default.resolve(outputDir, `subsidy_${numBatchesCalulated}.json`), JSON.stringify(thisProof, null, 4));
                numBatchesCalulated += 1;
                (0, utils_1.logYellow)(quiet, (0, utils_1.info)(`Progress: ${numBatchesCalulated} / ${totalSubsidyBatches}`));
            }
            catch (error) {
                (0, utils_1.logError)(error.message);
            }
        ...
        const subsidyFileData = {
            provider: process.env.ETH_PROVIDER || utils_1.DEFAULT_ETH_PROVIDER,
            maci: maciAddress,
            pollId: pollId.toString(),
            newSubsidyCommitment: (0, utils_1.asHex)(subsidyCircuitInputs.newSubsidyCommitment),
            results: {
                subsidy: poll.subsidy.map((x) => x.toString()),
                salt: (0, utils_1.asHex)(subsidyCircuitInputs.newSubsidySalt),
            },
        };
        // store it
        fs_1.default.writeFileSync(subsidyFile, JSON.stringify(subsidyFileData, null, 4));
        (0, utils_1.logYellow)(quiet, (0, utils_1.info)(`Subsidy file:\n${JSON.stringify(subsidyFileData, null, 4)}\n`));
        const susbsidyEndTime = Date.now();
        (0, utils_1.logYellow)(quiet, (0, utils_1.info)(`gen subsidy proof took ${(susbsidyEndTime - subsidyStartTime) / 1000} seconds\n`));
    }
    // tallying proofs
